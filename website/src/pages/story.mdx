...
We first used a BERT classic model. BLABLABLA WHY BERT
To do so here is our protocol:
- Compute the distances for the paths played by humans and by our machine

- Extact the pair of articles

- Compute for each of the pairs the embedding of each article

- Based on the embeddings, compute the cosine similarity and euclidean distance.

To complement this, we also added a SBERT model. BLABLABLA WHY